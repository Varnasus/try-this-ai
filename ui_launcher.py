        ...
        try:
            ...
            if last_check:
                ...
                if growth > GROWTH_ALERT_THRESHOLD:
                    logs.append(f"🚨 Growth alert: {entry['title']} — {views - last_views} new views in {hours_elapsed:.2f}h — {growth:.1f} views/hr")
        except Exception as e:
            logs.append(f"❌ Failed to fetch stats for {entry.get('youtube_video_id')}: {e}")
    return logs
...
with tab1:
    ...
    min_views = st.slider('Minimum views', min_value=0, max_value=10000, value=0, step=100)
...
with tab3:
    ...
    if queue:
        ...
        for i, e in enumerate(queue):
            ...
            e['title'] = st.text_input(f"Edit title {i}", value=e['title'], key=f"title_{i}")
            e['tags'] = st.text_input(f"Edit tags {i}", value=', '.join(e.get('tags', [])), key=f"tags_{i}").split(', ')
            thumbs = e.get('thumbnails', [])
            if thumbs:
                e['thumbnail_used'] = st.selectbox(f"Select thumbnail {i}", thumbs, key=f"thumb_{i}")
...
with tab4:
    ...
    entries = load_entries()
    engaged = [e for e in entries if e.get('uploaded') and all(k in e for k in ['views','likes','comments'])]
    if engaged:
        df = pd.DataFrame(engaged)
        df['engagement_rate'] = (df['likes'] + df['comments']) / df['views'] * 100
        st.dataframe(df[['title','views','likes','comments','engagement_rate']].sort_values(by='engagement_rate', ascending=False))
